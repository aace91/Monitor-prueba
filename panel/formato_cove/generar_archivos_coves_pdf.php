<?php
	...
	function generar_archivos_pdf_cove($referencia,$tipo_consulta){
		...
		if(!file_exists($directorio)){
			...
			if (!$result){
				$respuesta['Codigo'] = -1;
				$respuesta['Mensaje'] = "Generar PDF COVES :: Error al consultar la referencia. [".odbc_error($odbccasa).']['.$query.']'."\r\n";
				return $respuesta;
			}
			...
			if(!file_exists($directorio)){
				$respuesta['Codigo'] = -1;
				$respuesta['Mensaje'] = "Generar PDF COVES :: La referencia [".$referencia.",".$refo."] no existe en el directorio de los COVEs."."\r\n";
				return $respuesta;
			}
		}
		$respuesta['aCOVES'] = array(); $aComprobantes = array();
			...
			if(strpos(trim($dir_coves[$i]),'_COVEReturn.xml') !== false) {
				...
				foreach( $nodes as $node ){
					...
					if (trim($node->getElementsByTagName("mensajeInformativo")->item(0)->nodeValue) != 'La recepción del COVE fue exitosa.'){
						$respuesta['Codigo'] = -1;
						$respuesta['Mensaje'] .= "Generar PDF COVEs :: Generar PDF COVES :: Error de ventanilla para el COVE ".$dir_coves[$i].'['.trim($node->getElementsByTagName("mensajeInformativo")->item(0)->nodeValue).']'."\r\n";
						$bErrFile = true;
						//return $respuesta;
					}
						...
						for($j=0; $j<count($dir_coves); $j++){
							...
							if(strpos(trim($dir_coves[$j]),$xmlResp) !== false) {
								...
								foreach( $nodes as $node ){
									...
									if($error == 'true'){
										$respuesta['Codigo'] = -1;
										$respuesta['Mensaje'] .= "Generar PDF COVES :: El archivo XML del COVE contiene errores. NúmeroOperacón[".$numeroOperacion.']['.$dir_coves[$j].']'."\r\n";
										$bErrFile = true;
										break;
										//return $respuesta;
									}
										...
										foreach( $nodes as $node ){
											...
											if($aComprobantes[$c][2] == $Factura){
												...
												if (!$query){
													...
													$respuesta['Codigo'] = -1;
													$respuesta['Mensaje'] .= "Generar PDF COVES :: Error al consultar el RFC de consulta en nuestra base de datos. [".$RFC_Consulta."][".$error."][".$ArchCOVE."]"."\r\n";
													$bErrFile = true;
													//return $respuesta;
												}

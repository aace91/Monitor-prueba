...
if($loggedIn == false){
	...
	if (isset($_FILES['ifile_documentos']) && !empty($_FILES['ifile_documentos'])) { 
		...
		$sIdSalida = empty($_POST['sIdSalida']) ? '' : $_POST['sIdSalida'];
		...
		$carpeta = $sPathFiles . DIRECTORY_SEPARATOR . $sIdSalida;
		...
		for($i=0; $i < count($oFileNames); $i++){
			...
			$target = $sPathFiles . DIRECTORY_SEPARATOR . $sIdSalida . DIRECTORY_SEPARATOR . $sFileName;
			$respuesta['Target']=$target;
			if(move_uploaded_file($oFiles['tmp_name'][$i], $target)) {
				$success = true;
				$paths[] = $target;
				
				$consulta="INSERT INTO bodega.expos_salidas_files
								(id_salida, nombre_archivo, fecha)
								VALUES(".$sIdSalida.",
									   '".$sFileName."',
									   '".$fecha_registro."')";
		...
		/* ..:: Actualizar fecha documentacion ::.. */
		if($respuesta['Codigo'] == '1') {
			$consulta="UPDATE bodega.expos_salidas
					   SET documentado='".$fecha_registro."'
					   WHERE id_salida=".$sIdSalida;
								   
			$query = mysqli_query($cmysqli,$consulta);		
			if ($query==false){
				$error=mysql_error($cmysqli);
				$respuesta['Codigo']=-1;
		...
		/* ..:: Enviar correo ::.. */
		/* 21 Abril 2017 - A solicitud de carlos aguilar ya no se envian correos cuando sube factura */
		/*if($respuesta['Codigo'] == '1') {
			$sCorreos = new CCorreo;
			$sCorreos->GenerarEmails($sIdCliente, $sIdLogistica, $cmysqli);
			if ($sCorreos->bError) {
				$respuesta['Codigo']=-1;
				...
				envia_notificacion($adjuntos, $fec3, $sIdSalida, $fecha_registro, $sCorreos->bcc, $sCorreos->to);
			}
		}*/
	} else {
		$respuesta['Codigo']=-1;

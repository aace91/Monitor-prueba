...
if ($loggedIn == false){
		...
		array('db' => 'no_mov', 'dt' => 'refnumber', 'formatter' => function($val, $data) {
				if ($val==''){
					...
					if ($data['id_empresa'] == '1') {
						$nombre_fichero = "\\\\192.168.1.107\\gabdata\\Avanza\\gab\\pdf\\" . $val . ".pdf";
					} else if ($data['id_empresa'] == '2') {
						$nombre_fichero = "\\\\192.168.1.107\\gabdata\\Avanza\\sab\\pdf\\" . $val . ".pdf";
					} else {
						$nombre_fichero = "\\\\192.168.1.107\\gabdata\\Avanza\\gab\\pdf\\" . $val . ".pdf";
					}
	...
	$baseSql = "SELECT a.id_empresa, CONCAT(a.tipo_mov,'-',a.no_banco,'-',a.no_mov) AS cuenta_gastos, a.referencia, a.no_mov, b.rutadatos
				FROM expedientes.seguimiento_pedime AS a LEFT JOIN
					 expedientes.empresas AS b ON a.id_empresa = b.id_empresa

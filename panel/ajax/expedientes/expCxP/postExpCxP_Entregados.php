...
if ($loggedIn == false){
	...
	$columns = array(	
		...
		array('db' => 'fecha_cp_entrega', 'dt' => 'fecha_entrega' ),
    );
	...
	$baseSql = "SELECT DISTINCT(referencia) AS referencia,
	                   id_empresa,
					   CONCAT(tipo_mov,'-',no_banco,'-',no_mov) AS cuenta_gastos,
					   no_mov,
					   fecha_cp_entrega
				FROM expedientes.seguimiento_pedime
				WHERE fecha_cp_entrega='".$sFecha."'";

...
$baseSql = "
	SELECT
		...
		fac_mex.ttotal AS CARGOS_MEXICANOS,
		cf.tfac as TOTAL_FACTURAS
	FROM casa.saaio_pedime AS ped INNER JOIN 
       casa.saaio_factur AS fac_ped ON ped.NUM_REFE = fac_ped.NUM_REFE LEFT JOIN 
       bodega.remisiondet AS remd ON concat( ped.PAT_AGEN, '-', ped.NUM_PEDI) = remd.pedimento INNER JOIN 
       bodega.revision_general AS revg ON remd.referencia = revg.referencia AND
                                   fac_ped.NUM_FACT = revg.factura LEFT JOIN
       contagab.aacgmex AS fac_mex ON ped.ADU_DESP = fac_mex.aduana AND
                                      ped.PAT_AGEN = fac_mex.patente AND
                                      ped.NUM_PEDI = fac_mex.pedimento LEFT JOIN
       contagab.factura_consolidado AS ped_fac ON ped.ADU_DESP = ped_fac.aduana AND
                                                  ped.PAT_AGEN = ped_fac.patente AND
                                                  ped.NUM_PEDI = ped_fac.c001numped LEFT JOIN
       qbdelbravo_sync.invoicelinedetail AS fac_amed ON fac_amed.Description LIKE concat('%', remd.referencia, '%') LEFT JOIN
       qbdelbravo_sync.invoice AS fac_ame on fac_amed.IDKEY=fac_ame.TxnID LEFT JOIN (
				 SELECT count(f.num_fact) as tfac,f.num_refe
				 FROM casa.saaio_factur as f
				 GROUP BY f.num_refe) as cf on ped.num_refe=cf.num_refe
	WHERE ped.NUM_PEDI = '$num_pedi' and ped.PAT_AGEN='$patente' and ped.ADU_DESP='$aduana'
	GROUP BY fac_ped.num_fact
	ORDER BY NUM_FACT";

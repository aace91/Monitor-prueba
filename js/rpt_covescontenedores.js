...
function application_run() {
    ...
    try {
		...
		$('#itxt_referencia').on("keypress", function(e) {
			...
			if (e.keyCode == 13) {
				ajax_get_covecontenedores();
			}
		});

		fcn_cargar_grid_coves([]);
		fcn_cargar_grid_contenedores([]);
    } catch (err) {		
...
function fcn_cargar_grid_coves(aData) {
	try {
		show_custom_function_error('', 'idiv_mensaje');

		if (oTableCovesGrid == null) {
			var oDivDisplayErrors = 'idiv_mensaje';
			var div_table_name = 'dt_coves';
			var div_refresh_name = div_table_name + '_refresh';
			
			oTableCovesGrid = $('#' + div_table_name);
			
			oTableCovesGrid.DataTable({
				order: [[0, 'desc']],
				processing: false,
				serverSide: false,
				data: aData,
				columns: [ 
					{ "data": "factura" },
					{ "data": "cove", "className": "text-center" }
				],
				responsive: true,
				aLengthMenu: [
					[10, 25, 50, 100, 200, -1],
				...
				buttons: [
					{
						title: function() {
							return 'Reporte Coves ' + $('#itxt_referencia').val().trim()
						},
						extend: 'excel',
			...
			oTableCovesGrid.on( 'error.dt', function ( e, settings, techNote, message ) {
				on_grid_error(e, settings, techNote, message, oDivDisplayErrors);
			} );
		} else {
			oTableCovesGrid.DataTable().clear().draw();
			if (aData.length > 0) {
				oTableCovesGrid.dataTable().fnAddData(aData);	
			}
			setTimeout(function(){ oTableCovesGrid.DataTable().columns.adjust().responsive.recalc(); }, 500);
		}
	} catch (err) {		
		var strMensaje = 'fcn_cargar_grid_coves() :: ' + err.message;
		show_modal_error(strMensaje);
	}
}

function fcn_cargar_grid_contenedores(aData) {
	try {
		show_custom_function_error('', 'idiv_mensaje');
		if (oTableContenedoresGrid == null) {
			var oDivDisplayErrors = 'idiv_mensaje';
			var div_table_name = 'dt_contenedores';
			var div_refresh_name = div_table_name + '_refresh';
			
			oTableContenedoresGrid = $('#' + div_table_name);
			
			oTableContenedoresGrid.DataTable({
				order: [[0, 'desc']],
				processing: false,
				serverSide: false,
				data: aData,
				...
				buttons: [
					{
						title: function() {
							return 'Reporte Contenedores ' + $('#itxt_referencia').val().trim()
						},
						extend: 'excel',
						text: '<i class="fa fa-file-excel-o" aria-hidden="true"></i> Exportar Excel'
					}, 
			...
			oTableContenedoresGrid.on( 'error.dt', function ( e, settings, techNote, message ) {
				on_grid_error(e, settings, techNote, message, oDivDisplayErrors);
			} );
		} else {
			oTableContenedoresGrid.DataTable().clear().draw();
			if (aData.length > 0) {
				oTableContenedoresGrid.dataTable().fnAddData(aData);	
			}
			setTimeout(function(){ oTableContenedoresGrid.DataTable().columns.adjust().responsive.recalc(); }, 500);
		}
	} catch (err) {		
		var strMensaje = 'fcn_cargar_grid_contenedores() :: ' + err.message;
		show_modal_error(strMensaje);
	}
}
...
function ajax_get_covecontenedores() {
	try {	
		var sReferencia = $('#itxt_referencia').val().trim();
		if (sReferencia == '') {
			show_modal_error('Debe ingresar una referencia!');
			return false;
		}

		$.ajax({
			type: "POST",
			url: 'ajax/rpt_covescontenedores/rptCovesContenedoresFunc.php',
			data: {	
				action: 'consultar_covecontenedores',
			...
			success:  function (response) {
				...
				if (response != '500'){
					...
					if (respuesta.Codigo == '1'){
						fcn_cargar_grid_coves(respuesta.aCoves);
						fcn_cargar_grid_contenedores(respuesta.aContenedores);
					} else {
